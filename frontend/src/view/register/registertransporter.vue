...
import { registerTransporter } from '@/utils/api'

const router = useRouter()
const form = ref({ companyName: '', driverName: '', licensePlate: '', phone: '' })
...
async function register() {
  if (!form.value.companyName || !form.value.driverName) {
  ...
  try {
    ...
    const tx = await contract.registerSelf(Role.Transporter)
    ...
    localStorage.setItem('userRole', String(Role.Transporter))
    router.push({ name: 'TransporterView' })
  } catch (e: any) {
    ...
    <main class="main">
      <div class="card">
        <div class="card-header">
          <h1 class="card-title">Transporter</h1>
          <p class="card-desc">Register your logistics entity on the Organic Ledger.</p>
        </div>
        <div class="card-body">
          <div class="section-label"><span class="dot"></span>TRANSPORTER IDENTITY</div>
          <div class="field"><label>COMPANY NAME</label><input v-model="form.companyName" type="text"
              placeholder="e.g. Golden Valley Estates" /></div>
          <div class="field"><label>DRIVER NAME</label><input v-model="form.driverName" type="text"
              placeholder="Full Name" /></div>
          <div class="field"><label>LICENSE PLATE</label><input v-model="form.licensePlate" type="text"

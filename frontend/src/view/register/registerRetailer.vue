...
import { registerRetailer } from '@/utils/api'

const router = useRouter()
const form = ref({ storeName: '', ownerName: '', location: '', phone: '' })
...
async function register() {
  if (!form.value.storeName || !form.value.ownerName) {
  ...
  try {
    ...
    const tx = await contract.registerSelf(Role.Retailer)
    ...
    localStorage.setItem('userRole', String(Role.Retailer))
    router.push({ name: 'RetailerView' })
  } catch (e: any) {
    ...
    <main class="main">
      <div class="card">
        <div class="card-header">
          <h1 class="card-title">Retailer</h1>
          <p class="card-desc">Define your agricultural legacy. Your profile data will be anchored to the blockchain to
            ensure the provenance of every fruit harvested.</p>
        </div>
        <div class="card-body">
          <div class="section-label"><span class="dot"></span>RETAILER IDENTITY</div>
          <div class="field"><label>STORE NAME</label><input v-model="form.storeName" type="text"
              placeholder="e.g. Golden Valley Estates" /></div>
          <div class="field"><label>OWNER NAME</label><input v-model="form.ownerName" type="text"
              placeholder="Full Name" /></div>
          <div class="field"><label>LOCATION</label><input v-model="form.location" type="text" placeholder="Address" />
          </div>
          <div class="field"><label>PHONE</label><input v-model="form.phone" type="tel" placeholder="564-345-345" />
          </div>
